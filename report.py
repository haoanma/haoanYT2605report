...
def get_today_action_block(tickers=["QQQ", "TQQQ"]):
    try:
        ...
        for tk in tickers:
            ...
            bubble_threshold = 64.0 # 极值防线阈值 (钻石手版 64%)

            # 2. 边缘触发器 (Edge Triggers)
            ...
            if is_sell_shield1 or is_sell_shield2:
                if is_sell_shield1:
                    ...
                    desc = f"<b>衰退确认！</b>跌破年线且VIX飙升至 {curr['vix']:.1f}"
                else:
                    signal, color = "🚨 盾2：极值止盈", "#B42318"
                    desc = f"<b>泡沫刺破！</b>年线偏离达 {bias_pct:.1f}% 且跌穿20日均线"
                    
            elif is_buy_knife1 or is_buy_knife2 or is_buy_knife3:
                if is_buy_knife2:
                    signal, color = "🔥 刀2：左侧抄底", "#027A48"
                    desc = f"<b>非理性暴跌！</b>迎着极度恐慌 (VIX {curr['vix']:.1f}) 抢夺带血筹码"
                elif is_buy_knife1:
                    signal, color = "🚀 刀1：右侧顺势", "#027A48"
                    desc = "<b>长牛起航！</b>有效突破 MA200 年线牛熊分界"
                else:
                    signal, color = "⚡ 刀3：防踏空纠错", "#027A48"
